...
<div class="bg-white min-h-[calc(100vh-8rem)] rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow duration-300 p-8 lg:p-10 font-inter max-w-5xl mx-auto mb-10">
    ...
    <div class="mb-8 pb-5 border-b border-slate-100 flex items-center justify-between">
        <div>
            <h2 class="text-xl font-bold font-geist text-slate-800">Edit Data Siswa</h2>
            <p class="text-sm text-slate-500 font-inter mt-1">Perbarui informasi data peserta magang atas nama <span class="font-semibold text-slate-700">{{ $siswa->nama }}</span>.</p>
        </div>
    </div>
    ...
    <form action="{{ route('siswa.update', $siswa->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <section>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
                    <div>
                        ...
                        <input type="text" name="nama" value="{{ old('nama', $siswa->nama) }}" class="w-full bg-white border border-slate-200 rounded-lg px-4 py-2.5 text-sm text-slate-800 focus:border-primary focus:ring-2 focus:ring-primary/40 outline-none transition-all" required>
                    </div>

                    <div>
                        <label class="block text-[11px] font-semibold font-geist text-slate-500 uppercase tracking-wider mb-2">NIS</label>
                        <input type="text" name="nis" value="{{ old('nis', $siswa->nis ?? '') }}" class="w-full bg-white border border-slate-200 rounded-lg px-4 py-2.5 text-sm text-slate-800 focus:border-primary focus:ring-2 focus:ring-primary/40 outline-none transition-all" placeholder="Opsional (Bisa dikosongkan)">
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-2">
                            <input type="text" name="tempat_lahir" value="{{ old('tempat_lahir', $siswa->tempat_lahir) }}" class="w-1/2 bg-white border border-slate-200 rounded-lg px-4 py-2.5 text-sm text-slate-800 focus:border-primary focus:ring-2 focus:ring-primary/40 outline-none transition-all" placeholder="Tempat">
                            <input type="date" name="tanggal_lahir" value="{{ old('tanggal_lahir', $siswa->tanggal_lahir) }}" class="w-1/2 bg-white border border-slate-200 rounded-lg px-4 py-2.5 text-sm text-slate-800 focus:border-primary focus:ring-2 focus:ring-primary/40 outline-none transition-all">
                        </div>
                    </div>

                    <div>
                        <label class="block text-[11px] font-semibold font-geist text-slate-500 uppercase tracking-wider mb-2">Email</label>
                        <input type="email" name="email" value="{{ old('email', $siswa->email) }}" class="w-full bg-white border border-slate-200 rounded-lg px-4 py-2.5 text-sm text-slate-800 focus:border-primary focus:ring-2 focus:ring-primary/40 outline-none transition-all">
                    </div>

                    <div>
                        <label class="block text-[11px] font-semibold font-geist text-slate-500 uppercase tracking-wider mb-2">No HP</label>
                        <input type="text" name="no_hp" value="{{ old('no_hp', $siswa->no_hp) }}" class="w-full bg-white border border-slate-200 rounded-lg px-4 py-2.5 text-sm text-slate-800 focus:border-primary focus:ring-2 focus:ring-primary/40 outline-none transition-all">
                    </div>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea name="alamat" rows="3" class="w-full bg-white border border-slate-200 rounded-lg px-4 py-3 text-sm text-slate-800 focus:border-primary focus:ring-2 focus:ring-primary/40 outline-none transition-all resize-none">{{ old('alamat', $siswa->alamat) }}</textarea>
                    </div>
                </div>
            </section>
            ...
            <section>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
                    <div>
                        ...
                        <input type="text" name="asal_sekolah" value="{{ old('asal_sekolah', $siswa->asal_sekolah) }}" class="w-full bg-white border border-slate-200 rounded-lg px-4 py-2.5 text-sm text-slate-800 focus:border-primary focus:ring-2 focus:ring-primary/40 outline-none transition-all" required>
                    </div>

                    <div>
                        <label class="block text-[11px] font-semibold font-geist text-slate-500 uppercase tracking-wider mb-2">Jurusan <span class="text-red-500">*</span></label>
                        <input type="text" name="jurusan" value="{{ old('jurusan', $siswa->jurusan) }}" class="w-full bg-white border border-slate-200 rounded-lg px-4 py-2.5 text-sm text-slate-800 focus:border-primary focus:ring-2 focus:ring-primary/40 outline-none transition-all" required>
                    </div>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <select name="unit_penempatan" class="w-full bg-white border border-slate-200 rounded-lg px-4 py-2.5 text-sm text-slate-800 focus:border-primary focus:ring-2 focus:ring-primary/40 outline-none transition-all cursor-pointer">
                            ...
                            <option value="Subbagian Hukum" {{ (old('unit_penempatan', $siswa->unit_penempatan) == 'Subbagian Hukum') ? 'selected' : '' }}>Subbagian Hukum</option>
                            <option value="Subbagian Humas dan TU Kalan" {{ (old('unit_penempatan', $siswa->unit_penempatan) == 'Subbagian Humas dan TU Kalan') ? 'selected' : '' }}>Subbagian Humas dan TU Kalan</option>
                            <option value="Pemeriksa (Auditor)" {{ (old('unit_penempatan', $siswa->unit_penempatan) == 'Pemeriksa (Auditor)') ? 'selected' : '' }}>Pemeriksa (Auditor)</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-2">
                            <input type="date" name="tgl_mulai" value="{{ old('tgl_mulai', $siswa->tgl_mulai) }}" class="flex-1 bg-white border border-slate-200 rounded-lg px-4 py-2.5 text-sm text-slate-800 focus:border-primary focus:ring-2 focus:ring-primary/40 outline-none transition-all">
                            <span class="text-sm font-medium text-slate-400">s/d</span>
                            <input type="date" name="tgl_selesai" value="{{ old('tgl_selesai', $siswa->tgl_selesai) }}" class="flex-1 bg-white border border-slate-200 rounded-lg px-4 py-2.5 text-sm text-slate-800 focus:border-primary focus:ring-2 focus:ring-primary/40 outline-none transition-all">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="pembimbing" value="{{ old('pembimbing', $siswa->pembimbing) }}" class="w-full bg-white border border-slate-200 rounded-lg px-4 py-2.5 text-sm text-slate-800 focus:border-primary focus:ring-2 focus:ring-primary/40 outline-none transition-all">
                    </div>
                </div>
            </section>
                    ...
                    <div class="flex flex-col gap-3">
                        <div class="flex gap-4 items-center">
                            ...
                            <div class="w-20 h-24 rounded-lg bg-slate-100 border border-slate-200 overflow-hidden shrink-0 relative flex items-center justify-center">
                                @if($siswa->pas_foto)
                                    <img id="preview-img" src="{{ asset('storage/' . $siswa->pas_foto) }}" class="w-full h-full object-cover">
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="space-y-5">
                        <div>
                            ...
                            <textarea name="rekam_jejak" rows="3" class="w-full bg-white border border-slate-200 rounded-lg px-4 py-3 text-sm text-slate-800 focus:border-primary focus:ring-2 focus:ring-primary/40 outline-none transition-all resize-none">{{ old('rekam_jejak', $siswa->rekam_jejak) }}</textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="catatan_khusus" value="{{ old('catatan_khusus', $siswa->catatan_khusus) }}" class="w-full bg-white border border-slate-200 rounded-lg px-4 py-2.5 text-sm text-slate-800 focus:border-primary focus:ring-2 focus:ring-primary/40 outline-none transition-all">
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="mt-12 flex items-center justify-end gap-3 pt-6 border-t border-slate-200">
            <a href="{{ route('siswa.index') }}" class="inline-flex items-center justify-center bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 font-medium font-geist rounded-lg px-5 py-2.5 text-sm transition-colors shadow-sm">
                Batal
            </a>
