...
<div class="bg-white min-h-[calc(100vh-8rem)] rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow duration-300 p-8 lg:p-10 font-inter max-w-5xl mx-auto mb-10">
    ...
    <div class="mb-8 pb-5 border-b border-slate-100 flex items-center justify-between">
        <div>
            <h2 class="text-xl font-bold font-geist text-slate-800">Edit Data Mahasiswa</h2>
            <p class="text-sm text-slate-500 font-inter mt-1">Perbarui informasi data peserta magang atas nama <span class="font-semibold text-slate-700">{{ $mahasiswa->nama }}</span>.</p>
        </div>
    </div>
    ...
    <form action="{{ route('mahasiswa.update', $mahasiswa->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="space-y-12">
            ...
            <section>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
                    <div>
                        ...
                        <input type="text" name="nama" value="{{ old('nama', $mahasiswa->nama) }}" class="w-full bg-white border border-slate-200 rounded-lg px-4 py-2.5 text-sm text-slate-800 focus:border-primary focus:ring-2 focus:ring-primary/40 outline-none transition-all" required>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-2">
                            <input type="text" name="tempat_lahir" value="{{ old('tempat_lahir', $mahasiswa->tempat_lahir) }}" class="w-1/2 bg-white border border-slate-200 rounded-lg px-4 py-2.5 text-sm text-slate-800 focus:border-primary focus:ring-2 focus:ring-primary/40 outline-none transition-all" placeholder="Tempat">
                            <input type="date" name="tanggal_lahir" value="{{ old('tanggal_lahir', $mahasiswa->tanggal_lahir) }}" class="w-1/2 bg-white border border-slate-200 rounded-lg px-4 py-2.5 text-sm text-slate-800 focus:border-primary focus:ring-2 focus:ring-primary/40 outline-none transition-all">
                        </div>
                    </div>

                    <div>
                        <label class="block text-[11px] font-semibold font-geist text-slate-500 uppercase tracking-wider mb-2">Email</label>
                        <input type="email" name="email" value="{{ old('email', $mahasiswa->email) }}" class="w-full bg-white border border-slate-200 rounded-lg px-4 py-2.5 text-sm text-slate-800 focus:border-primary focus:ring-2 focus:ring-primary/40 outline-none transition-all">
                    </div>

                    <div>
                        <label class="block text-[11px] font-semibold font-geist text-slate-500 uppercase tracking-wider mb-2">No HP</label>
                        <input type="text" name="no_hp" value="{{ old('no_hp', $mahasiswa->no_hp) }}" class="w-full bg-white border border-slate-200 rounded-lg px-4 py-2.5 text-sm text-slate-800 focus:border-primary focus:ring-2 focus:ring-primary/40 outline-none transition-all">
                    </div>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea name="alamat" rows="3" class="w-full bg-white border border-slate-200 rounded-lg px-4 py-3 text-sm text-slate-800 focus:border-primary focus:ring-2 focus:ring-primary/40 outline-none transition-all resize-none">{{ old('alamat', $mahasiswa->alamat) }}</textarea>
                    </div>
                </div>
            </section>
            ...
            <section>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
                    <div>
                        <label class="block text-[11px] font-semibold font-geist text-slate-500 uppercase tracking-wider mb-2">Asal Universitas / Perguruan Tinggi <span class="text-red-500">*</span></label>
                        <input type="text" name="asal_kampus" value="{{ old('asal_kampus', $mahasiswa->asal_kampus) }}" class="w-full bg-white border border-slate-200 rounded-lg px-4 py-2.5 text-sm text-slate-800 focus:border-primary focus:ring-2 focus:ring-primary/40 outline-none transition-all" required>
                    </div>

                    <div>
                        <label class="block text-[11px] font-semibold font-geist text-slate-500 uppercase tracking-wider mb-2">Program Studi / Jurusan <span class="text-red-500">*</span></label>
                        <input type="text" name="prodi" value="{{ old('prodi', $mahasiswa->prodi) }}" class="w-full bg-white border border-slate-200 rounded-lg px-4 py-2.5 text-sm text-slate-800 focus:border-primary focus:ring-2 focus:ring-primary/40 outline-none transition-all" required>
                    </div>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <select name="unit_penempatan" class="w-full bg-white border border-slate-200 rounded-lg px-4 py-2.5 text-sm text-slate-800 focus:border-primary focus:ring-2 focus:ring-primary/40 outline-none transition-all cursor-pointer">
                            ...
                            <option value="Subbagian SDM" {{ (old('unit_penempatan', $mahasiswa->unit_penempatan) == 'Subbagian SDM') ? 'selected' : '' }}>Subbagian SDM</option>
                            <option value="Subbagian Umum dan TI" {{ (old('unit_penempatan', $mahasiswa->unit_penempatan) == 'Subbagian Umum dan TI') ? 'selected' : '' }}>Subbagian Umum dan TI</option>
                            <option value="Subbagian Keuangan" {{ (old('unit_penempatan', $mahasiswa->unit_penempatan) == 'Subbagian Keuangan') ? 'selected' : '' }}>Subbagian Keuangan</option>
                            <option value="Subbagian Hukum" {{ (old('unit_penempatan', $mahasiswa->unit_penempatan) == 'Subbagian Hukum') ? 'selected' : '' }}>Subbagian Hukum</option>
                            <option value="Subbagian Humas dan TU Kalan" {{ (old('unit_penempatan', $mahasiswa->unit_penempatan) == 'Subbagian Humas dan TU Kalan') ? 'selected' : '' }}>Subbagian Humas dan TU Kalan</option>
                            <option value="Pemeriksa (Auditor)" {{ (old('unit_penempatan', $mahasiswa->unit_penempatan) == 'Pemeriksa (Auditor)') ? 'selected' : '' }}>Pemeriksa (Auditor)</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-2">
                            <input type="date" name="tgl_mulai" value="{{ old('tgl_mulai', $mahasiswa->tgl_mulai) }}" class="flex-1 bg-white border border-slate-200 rounded-lg px-4 py-2.5 text-sm text-slate-800 focus:border-primary focus:ring-2 focus:ring-primary/40 outline-none transition-all">
                            <span class="text-sm font-medium text-slate-400">s/d</span>
                            <input type="date" name="tgl_selesai" value="{{ old('tgl_selesai', $mahasiswa->tgl_selesai) }}" class="flex-1 bg-white border border-slate-200 rounded-lg px-4 py-2.5 text-sm text-slate-800 focus:border-primary focus:ring-2 focus:ring-primary/40 outline-none transition-all">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="pembimbing" value="{{ old('pembimbing', $mahasiswa->pembimbing) }}" class="w-full bg-white border border-slate-200 rounded-lg px-4 py-2.5 text-sm text-slate-800 focus:border-primary focus:ring-2 focus:ring-primary/40 outline-none transition-all">
                    </div>
                </div>
            </section>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div class="md:col-span-1">
                    ...
                    <p class="text-xs text-slate-500 mb-4">Format JPG/PNG. Biarkan kosong jika tidak ingin mengubah foto.</p>
                    ...
                    <div class="flex flex-col gap-3">
                        <div class="flex gap-4 items-center">
                            ...
                            <div class="w-20 h-24 rounded-lg bg-slate-100 border border-slate-200 overflow-hidden shrink-0 relative flex items-center justify-center">
                                @if($mahasiswa->pas_foto)
                                    <img id="preview-img" src="{{ asset('storage/' . $mahasiswa->pas_foto) }}" class="w-full h-full object-cover">
                                    <span id="preview-placeholder" class="text-[10px] font-medium text-slate-400 hidden">3 x 4</span>
                                @else
                            ...
                            <label class="flex-1 border-2 border-dashed border-slate-300 rounded-xl p-4 flex flex-col items-center justify-center text-center hover:bg-slate-50 hover:border-primary transition-all cursor-pointer group bg-white h-24">
                                ...
                                <div class="text-xs font-medium font-geist text-primary">Ganti Foto</div>
                        ...
                        <div class="flex items-center gap-2 mt-1">
                            <input type="checkbox" name="hapus_foto" value="1" id="hapus_foto" class="w-4 h-4 text-red-500 rounded border-slate-300 focus:ring-red-500 cursor-pointer">
                            <label for="hapus_foto" class="text-xs text-red-500 font-medium cursor-pointer">Hapus foto saat ini</label>
                        </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="space-y-5">
                        <div>
                            ...
                            <textarea name="rekam_jejak" rows="3" class="w-full bg-white border border-slate-200 rounded-lg px-4 py-3 text-sm text-slate-800 focus:border-primary focus:ring-2 focus:ring-primary/40 outline-none transition-all resize-none">{{ old('rekam_jejak', $mahasiswa->rekam_jejak) }}</textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="catatan_khusus" value="{{ old('catatan_khusus', $mahasiswa->catatan_khusus) }}" class="w-full bg-white border border-slate-200 rounded-lg px-4 py-2.5 text-sm text-slate-800 focus:border-primary focus:ring-2 focus:ring-primary/40 outline-none transition-all">
                        </div>
                    </div>
                </div>
            </div>
