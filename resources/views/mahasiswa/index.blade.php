...
<div class="mb-6 flex items-center justify-between">
    <div>
        <h2 class="text-2xl font-bold font-geist text-slate-800">Manajemen Data Mahasiswa</h2>
        <p class="text-sm text-slate-500 font-inter mt-1">Kelola data mahasiswa perguruan tinggi yang terdaftar di instansi.</p>
    </div>
</div>
...
<div class="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow duration-300 border border-slate-100 p-4 mb-8 flex flex-col xl:flex-row xl:items-center justify-between gap-4">
    <div class="flex flex-col md:flex-row items-center gap-3 w-full xl:w-auto">
        ...
        <div class="relative w-full md:w-64">
            ...
            <input type="text" id="searchInput" class="block w-full pl-9 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-800 placeholder-slate-400 focus:border-primary focus:ring-2 focus:ring-primary/40 focus:bg-white outline-none transition-all font-inter" placeholder="Cari nama atau NIM...">
        </div>
    ...
    <!-- Tambah Button -->
    <a href="{{ route('mahasiswa.create') }}" class="inline-flex items-center justify-center bg-primary hover:bg-primary-hover text-white font-medium font-geist rounded-lg px-5 py-2 text-sm transition-colors shadow-sm focus:ring-2 focus:ring-primary/40 whitespace-nowrap">
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8" id="cardContainer">
    ...
    @forelse($mahasiswa as $mhs)
        @php
            ...
            // Logika Status Baru
            if (empty($mhs->tgl_selesai)) {
                $status = 'Selesai';
                $statusDot = 'bg-slate-400';
            } elseif ($mhs->tgl_mulai > $today) {
                $status = 'Akan Masuk';
                $statusDot = 'bg-amber-500';
            } elseif ($mhs->tgl_mulai <= $today && $mhs->tgl_selesai >= $today) {
                $status = 'Aktif';
        ...
        <!-- CARD MAHASISWA (Ditambahkan class 'data-card' dan 'data-*' attributes) -->
        <div class="data-card bg-white rounded-[16px] shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 border border-slate-100 flex flex-col group" 
             ...
             data-identitas="{{ strtolower($mhs->nim) }}" 
             data-status="{{ $status }}" 
            ...
            <div class="p-6 flex gap-6">
                ...
                <div class="shrink-0 flex flex-col items-center gap-3">
                    <div class="w-[96px] h-[128px] bg-[#eaf1ff] border border-slate-200 rounded-lg flex flex-col items-center justify-center text-center relative overflow-hidden">
                        @if($mhs->pas_foto)
                            <img src="{{ asset('storage/' . $mhs->pas_foto) }}" alt="Foto {{ $mhs->nama }}" class="w-full h-full object-cover">
                        @else
                ...
                <div class="flex-1 min-w-0 py-1">
                    <h3 class="text-lg font-bold font-geist text-primary truncate leading-tight mb-1">{{ $mhs->nama }}</h3>
                    <div class="text-xs font-inter text-slate-500 mb-1">NIM: <span class="text-slate-800 font-medium">{{ $mhs->nim }}</span></div>
                    <div class="text-sm font-bold font-inter text-slate-800 mb-2 truncate">{{ $mhs->asal_kampus }}</div>
                    
                    <div class="text-xs font-inter text-slate-500 leading-tight mb-0.5">
                        {{ $mhs->tempat_lahir ?? '-' }}, {{ $mhs->tanggal_lahir ? \Carbon\Carbon::parse($mhs->tanggal_lahir)->locale('id')->isoFormat('D MMMM Y') : '-' }}
                    </div>
                    <div class="text-xs font-inter text-slate-500 leading-tight mb-0.5">
                        {{ $mhs->jenis_kelamin == 'L' ? 'Laki-laki' : ($mhs->jenis_kelamin == 'P' ? 'Perempuan' : '-') }}
                    </div>
                    <div class="text-xs font-semibold font-inter text-slate-700 leading-tight mt-1">{{ $mhs->no_hp ?? '-' }}</div>
                    ...
                    <div class="mt-4">
                        ...
                        <div class="text-xs font-inter text-slate-600 font-medium">
                            {{ $mhs->tgl_mulai ? \Carbon\Carbon::parse($mhs->tgl_mulai)->locale('id')->isoFormat('D MMMM Y') : '-' }} 
                            <span class="font-normal text-slate-400 mx-1">s/d</span> 
                            {{ $mhs->tgl_selesai ? \Carbon\Carbon::parse($mhs->tgl_selesai)->locale('id')->isoFormat('D MMMM Y') : '-' }}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="relative mt-auto px-6 py-3.5 border-t border-slate-100 flex items-center justify-between bg-slate-50/50 rounded-b-[16px]">
                ...
                <a href="{{ route('mahasiswa.show', $mhs->id) }}" class="absolute left-1/2 -translate-x-1/2 inline-flex items-center text-sm font-semibold font-geist text-primary hover:text-primary-hover transition-colors whitespace-nowrap">
                ...
                <div class="flex items-center gap-2">
                    <a href="{{ route('mahasiswa.edit', $mhs->id) }}" class="p-2 text-slate-400 hover:text-amber-500 transition-colors" title="Edit">
                    ...
                    <form action="{{ url('/mahasiswa/' . $mhs->id) }}" method="POST" class="inline-block form-delete" data-nama="{{ $mhs->nama }}">
                        @csrf
                        @method('DELETE')
    ...
    <div id="emptyState" class="col-span-1 lg:col-span-2 bg-white rounded-[16px] shadow-sm border border-slate-200 p-12 flex flex-col items-center justify-center text-center {{ $mahasiswa->count() > 0 ? 'hidden' : 'flex' }}">
