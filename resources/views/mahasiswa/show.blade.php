...
@php
    ...
    // Logika Status
    if (empty($mahasiswa->tgl_selesai)) {
        $status = 'Selesai';
        $badgeColor = 'bg-slate-100 text-slate-600 border-slate-200';
        $dotColor = 'bg-slate-400';
    } elseif ($mahasiswa->tgl_mulai > $today) {
        $status = 'Akan Masuk';
        $badgeColor = 'bg-amber-50 text-amber-600 border-amber-200';
        $dotColor = 'bg-amber-500';
    } elseif ($mahasiswa->tgl_mulai <= $today && $mahasiswa->tgl_selesai >= $today) {
        $status = 'Aktif';
...
<div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <a href="{{ route('mahasiswa.index') }}" class="inline-flex items-center text-sm font-medium font-inter text-slate-500 hover:text-slate-800 transition-colors">
    ...
    <div class="flex items-center gap-3">
        ...
        <div class="flex items-center bg-white border border-slate-100 rounded-lg p-1 shadow-sm hover:shadow-md transition-shadow duration-300">
            ...
            <a href="{{ route('mahasiswa.id_card', $mahasiswa->id) }}" target="_blank" class="inline-flex items-center justify-center text-slate-600 hover:bg-indigo-50 hover:text-indigo-600 font-medium font-inter rounded-md px-3 py-1.5 text-xs transition-colors" title="Cetak ID Card Magang">
            ...
            <a href="{{ route('mahasiswa.biodata', $mahasiswa->id) }}" target="_blank" class="inline-flex items-center justify-center text-slate-600 hover:bg-blue-50 hover:text-blue-600 font-medium font-inter rounded-md px-3 py-1.5 text-xs transition-colors" title="Cetak Formulir Biodata">
            ...
            @if(empty($mahasiswa->tgl_selesai) || $mahasiswa->tgl_selesai < \Carbon\Carbon::today()->toDateString())
                <div class="w-px h-4 bg-slate-200 mx-1"></div>
                <a href="{{ route('mahasiswa.sertifikat', $mahasiswa->id) }}" target="_blank" class="inline-flex items-center justify-center text-slate-600 hover:bg-amber-50 hover:text-amber-600 font-medium font-inter rounded-md px-3 py-1.5 text-xs transition-colors" title="Cetak Sertifikat Selesai Magang">
        ...
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow duration-300 p-6 flex flex-col items-center text-center">
            <div class="w-32 h-40 bg-[#eaf1ff] border border-slate-200 rounded-xl mb-4 relative overflow-hidden flex items-center justify-center shadow-inner">
                @if($mahasiswa->pas_foto)
                    <img src="{{ asset('storage/' . $mahasiswa->pas_foto) }}" alt="Foto" class="w-full h-full object-cover">
                @else
                    <span class="text-sm font-medium font-inter text-slate-400">3 x 4</span>
                @endif
            </div>
            
            <h2 class="text-xl font-bold font-geist text-slate-800 leading-tight mb-1">{{ $mahasiswa->nama }}</h2>
            <p class="text-sm font-inter text-slate-500 mb-4">{{ $mahasiswa->nim }}</p>
        ...
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow duration-300 p-6">
            ...
            <ul class="space-y-4 font-inter text-sm">
                <li class="flex items-start gap-3 text-slate-600">
                    ...
                    <span class="break-all">{{ $mahasiswa->email ?? 'Tidak ada email' }}</span>
                </li>
                <li class="flex items-start gap-3 text-slate-600">
                    ...
                    <span>{{ $mahasiswa->no_hp ?? 'Tidak ada No. HP' }}</span>
                </li>
                <li class="flex items-start gap-3 text-slate-600">
                    <svg class="w-5 h-5 text-slate-400 shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
                    <span>{{ $mahasiswa->alamat ?? 'Alamat belum diatur' }}</span>
                </li>
            </ul>
        </div>
    </div>
    ...
    <div class="md:col-span-2 space-y-6">
        ...
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow duration-300 p-6 sm:p-8">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-y-6 gap-x-8 font-inter">
                <div>
                    <p class="text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-1">Asal Kampus</p>
                    <p class="text-sm font-medium text-slate-800">{{ $mahasiswa->asal_kampus }}</p>
                </div>
                <div>
                    <p class="text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-1">Program Studi</p>
                    <p class="text-sm font-medium text-slate-800">{{ $mahasiswa->prodi }}</p>
                </div>
                ...
                <div>
                    ...
                    <p class="text-sm font-medium text-slate-800">{{ $mahasiswa->pembimbing ?? '-' }}</p>
                </div>
                <div>
        ...
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow duration-300 p-6 sm:p-8">
            ...
            <div class="space-y-6 font-inter">
                <div>
                    ...
                    <div class="bg-slate-50 border border-slate-100 rounded-lg p-4 text-sm text-slate-700 whitespace-pre-wrap leading-relaxed">{{ $mahasiswa->rekam_jejak ?: 'Tidak ada rekam jejak yang dicatat.' }}</div>
                </div>
                <div>
                    <p class="text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-2">Catatan Khusus</p>
                    <p class="text-sm font-medium text-amber-600">{{ $mahasiswa->catatan_khusus ?: '-' }}</p>
                </div>
            </div>
        </div>
...
<!-- ================= FOOTER AKSI BAWAH ================= -->
    <div class="mt-8 flex items-center justify-end gap-3 pt-6 border-t border-slate-200">
        <a href="{{ route('mahasiswa.edit', $mahasiswa->id) }}" class="inline-flex items-center justify-center bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 font-medium font-geist rounded-lg px-5 py-2.5 text-sm transition-colors shadow-sm">
        ...
        <form action="{{ route('mahasiswa.destroy', $mahasiswa->id) }}" method="POST" class="form-delete" data-nama="{{ $mahasiswa->nama }}">
            @csrf
            @method('DELETE')
